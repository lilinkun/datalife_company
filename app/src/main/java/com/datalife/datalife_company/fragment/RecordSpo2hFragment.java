...
public class RecordSpo2hFragment extends BaseRecordFragment implements RecordContract,OnChartValueSelectedListener, OnChartGestureListener {
...
//        xAxis.setLabelCount(6,true);
    ...
    private void setData(int count) {
        ...
        if (count>0){
            ...
            smallStateView.setValue(IDatalifeConstant.getWM(getActivity()),Double.parseDouble(measureRecordBeans.get(measureRecordBeans.size()-1).getCheckValue1()), MeasureNorm.getSPO2H(),"");
        }

        //设置一页最大显示个数为6，超出部分就滑动
        if (count > 6){
            if (ratio != (float) count/(float) 6){
...
//            set1.setDrawCircleHole(false);
    ...
    public void onSuccess(ArrayList<MeasureRecordBean> newsInfos) {
        ...
        this.measureRecordBeans = newsInfos;
    ...
    public void getRecordData(){
        if (recordPresenter != null){
            recordPresenter.onGetListValue(String.valueOf(1), RecordActivity.ShowCount, RecordActivity.memberID, IDatalifeConstant.MACHINE_HEALTH, String.valueOf(IDatalifeConstant.SPO2HINT), "", "", ProApplication.SESSIONID);
        }
    }
